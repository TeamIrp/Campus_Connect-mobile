...
class PublicationDetailsScreen extends StatefulWidget {
  final PublicationDatum publication;

  const PublicationDetailsScreen({super.key, required this.publication});
...
class _PublicationDetailsScreenState extends State<PublicationDetailsScreen> {
  final LatLng eventLocation = LatLng(37.7749, -122.4194); // Dummy location
  bool isLiked = false;
  ...
  Widget build(BuildContext context) {
    ...
    final imageUrl = (pub.publicationimage != null && pub.publicationimage!.isNotEmpty)
        ? '${ConstantUrl.baseUrl}${pub.publicationimage!}'
        : null;
    final title = pub.publicationTitle ?? 'Untitled';
    final categoryText = pub.publicationCategory ?? '';
    final date = pub.publicationDate != null
        ? pub.publicationDate!.toLocal().toString().split(' ')[0]
        : '';
    final description = pub.publicationDescription ?? 'No description available.';
                  ...
                  children: [
                    SizedBox(
                      ...
                      child: imageUrl != null
                          ...
                          errorBuilder: (_, __, ___) => Image.asset('assets/images/publication_image1.png', fit: BoxFit.cover))
                        ...
                        children: [
                          IconButton(
                            icon: Icon(isLiked ? Icons.thumb_up : Icons.thumb_up_outlined,
                                size: 28, color: isLiked ? const Color(0xFF1D97D4) : Colors.white),
                            onPressed: _toggleLike,
                          ),
                ...
                Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      const SizedBox(height: 20),
                      GestureDetector(
                        onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const AdvertisementScreen())),
                        child: Container(
                          height: 80,
                          decoration: BoxDecoration(
